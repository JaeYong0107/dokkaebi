...
5. **디자인에 영향이 있는 수정**은 반드시 `gstack browse` CLI로 모바일(375x812) / 태블릿(768) / 데스크탑(1280) 뷰를 직접 확인하며 UI를 최적화한다 (반응형 깨짐·텍스트 정렬·스티키/드롭다운 동작 등). 스크린샷으로 검증하지 않은 디자인 변경은 완료 처리하지 않는다.
